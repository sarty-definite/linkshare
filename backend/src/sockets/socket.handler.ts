    ...
    try {
      ...
      next(
        error instanceof Error
          ? error
          : new Error("Socket authorization failed"),
      );
    }
  });
  ...
  io.on("connection", async (socket) => {
    ...
    if (activeSet.size >= env.ROOM_MAX_SESSIONS) {
      socket.emit("room:error", {
        message: "Maximum sessions limit reached for this room.",
      });
    ...
    io.to(roomId).emit("room:presence", {
      roomId,
      presenceCount: activeSet.size,
    });
    CleanupService.scheduleRoomCleanup(
      roomId,
    ...
    socket.on("disconnect", async () => {
      ...
      if (!roomSockets || roomSockets.size === 0) {
        ...
        await YjsRoomManager.forcePersistAndCleanup(roomId).catch(
          () => undefined,
        );

        const lastActivityAt = new Date();
        await RoomRepository.update(roomId, { lastActivityAt }).catch(
          () => undefined,
        );
      ...
      io.to(roomId).emit("room:presence", {
        roomId,
        presenceCount: RoomService.getPresenceCount(roomId),
      });
    });
  });
}
